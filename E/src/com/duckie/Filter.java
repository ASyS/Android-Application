...
public class Filter {
	...
	public static Mat subtractBG(Mat src, Mat rgb) {
		...
		// Core.rectangle(rgb, boundRect.tl(), boundRect.br(), new Scalar(255,
		// 255, 0), 2, 8, 0 );

		// Create a mask for the contour to mask out that region from image.
		Mat mask = Mat.zeros(src.size(), CvType.CV_8UC1);
		Imgproc.drawContours(mask, contours, indexOfBiggestContour, new Scalar(
				255), -1);

		// At this point, mask has value of 255 for pixels within the contour
		// and value of 0 for those not in contour.
	...
	public static Mat furtherClean(Mat image) {
