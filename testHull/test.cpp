...
int main(){
	...
	for(int i = 0; i < 22; i++){
		...
		templateContours[i] = findObjectHull2(src, processed);
	}
 ...
 */
Mat subtractBG(Mat im){
	Mat image = im.clone();
	Mat subtracted = Mat::zeros(image.size(), CV_8UC3);
	cvtColor(image, image, CV_BGR2HSV);
//	imshow("HSV", image);
	inRange(image, Scalar(100, 80, 60), Scalar(125, 255, 255), image);
	threshold(image, image, 0, 255, THRESH_BINARY_INV);
//	im.copyTo(subtracted, image);
//	imshow("Blue Filtered", subtracted);
//
//	cvtColor(subtracted, subtracted, CV_BGR2HSV);
//	inRange(subtracted, Scalar(0, 0, 76), Scalar(0, 0, 255), subtracted);
//	imshow("black filtered?", subtracted);
